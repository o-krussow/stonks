...
def strategy1(ticker_name):
    print(ticker_name)


#nasdaq tickers only for now, for testing-----------
nasdaq_tickers = read_tickers_from_file("nasdaq_tickers.txt")
objectified_nasdaq_tickers = [Ticker.Ticker(ticker_name, random.randint(0, 10)) for ticker_name in nasdaq_tickers]  #Making a list of Ticker objects with a random quantity from the original list of ticker strings
#----------------------------------------------------
